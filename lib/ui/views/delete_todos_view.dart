...
class DeleteTodosView extends StatefulWidget {
  ...
  final int index;
  const DeleteTodosView({Key? key, required this.todos, required this.index}) : super(key: key);
...
class _DeleteTodosViewState extends State<DeleteTodosView> {
  ...
  int get selectedItemsCount => checkedBoxStatus.where((status) => status == true).length;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          selectedItemsCount == 0 ? 'Please select items' : '$selectedItemsCount selected items',
          ...
          Checkbox(
              ...
              onChanged: (value) {
                setState(() {
                  isChecked = value!;
                  if (isChecked) {
                    checkedBoxStatus = List.generate(widget.todos.length, (index) => true);
                  } else {
                    checkedBoxStatus = List.generate(widget.todos.length, (index) => false);
                  }
                });
