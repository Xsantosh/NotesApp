...
class _NoteViewState extends State<NoteView> {
  ...
  Widget build(BuildContext context) {
    final isLoading =
        context.select<NoteViewModel, bool>((noteViewModel) => noteViewModel.isLoading);
    final notes = context.select<NoteViewModel, List<Note>>((noteViewModel) => noteViewModel.notes);
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text('Trevor Dallas\'s Note'),
          ...
          child: Padding(
            ...
            child: Column(
              children: <Widget>[
                ...
                SearchBar(controller: searchNoteController, hintText: 'Search Notes'),
                SizedBox(height: 20.h),
                Expanded(
                  child: isLoading
                      ? Center(
                      ...
                      : !isLoading && notes.isEmpty //context.watch<NoteViewModel>().notes.isEmpty
                          ? Center(child: Text("No Note"))
                          : ListView.builder(
                              itemBuilder: (context, index) => GestureDetector(
                                onTap: () {
                                  Navigator.of(context)
                                      .pushNamed(Routes.editNoteRoute, arguments: notes[index]);
